...
class ADDA(pl.LightningModule):
    ...
    def forward(self, dirty_spec, mask: torch.tensor | None = None):
    ...
    def process_batch(self, batch, step: str = "train"):
        ...
        if not self.padded_spectra:
            ...
            mask, clean_enc_spec, dirty_spec = batch
        batch_size = clean_enc_spec.shape[0]
        ...
        clean_d_loss = self.loss(clean_classification, clean)
        dirty_d_loss = self.loss(dirty_classification, dirty)
        ...
        ## train encoder
        dirty_classification = self.classifier(dirty_enc_spec)
        g_loss = self.loss(dirty_classification, clean)
