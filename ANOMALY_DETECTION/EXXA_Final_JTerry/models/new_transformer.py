...
class TransformerEncoder(TransformerBlock):
    ...
    def forward(self, src, mask: torch.TensorType | None = None) -> torch.Tensor:
...
class TransformerDecoder(TransformerBlock):
    ...
    def forward(self, trg, enc_src, mask: torch.TensorType | None = None) -> torch.Tensor:
        # Check for empty input tensor
        ...
        for layer in self.layers:
            trg = layer(
                trg, memory=enc_src, tgt_key_padding_mask=mask, memory_key_padding_mask=mask
            )
...
class IntegratedTransformerSeq2Seq(pl.LightningModule):
    ...
    def forward(self, spectrum, mask: torch.TensorType | None = None) -> torch.Tensor:
        ...
        enc_src = self.encoder(spectrum, mask=mask)
        trg = torch.zeros_like(spectrum).to(self.device) if self.trg_eq_zero else spectrum
        outputs = self.decoder(trg, enc_src, mask=mask)
    ...
    def process_batch(self, batch, step: str = "train"):
        if not self.padded_spectrum:
            ...
            padded_mask = mask.to(self.device)
        ...
        if self.add_noise:
            ...
            gen_data = self(spectrum + noise, padded_mask)
        else:
            gen_data = self(spectrum, padded_mask)

        loss = (
            self.loss(spectrum, gen_data)
            if not self.padded_spectrum
            else self.masked_mse_loss(gen_data, spectrum, mask)
        )
        self.log(f"{step}_loss", loss)
        return loss

    def masked_mse_loss(self, y_pred, y_true, mask):
        # Using masked_select to get non-padded parts of output and target
