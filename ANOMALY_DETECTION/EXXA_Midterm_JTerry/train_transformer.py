...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_dim", type=int, default=1, help="Transformer hidden diemsnion")
    parser.add_argument(
        "--num_layers", type=int, default=3, help="Number of transformer encoding layers"
    )

    parser.add_argument(
        "--num_heads", type=int, default=1, help="Number of transformer encoding layers"
    )
    ...
    if model_hparams["hidden_dim"] % model_hparams["num_heads"] != 0:
        if model_hparams["hidden_dim"] < model_hparams["num_heads"]:
            model_hparams["num_heads"] = model_hparams["hidden_dim"]
        else:
            # model_hparams["num_heads"] = max([1, model_hparams["hidden_dim"] // model_hparams["num_heads"]])
            while model_hparams["hidden_dim"] % model_hparams["num_heads"] != 0:
