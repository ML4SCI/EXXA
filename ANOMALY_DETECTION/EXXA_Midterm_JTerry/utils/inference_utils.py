...
from utils.data_utils import fill_spectrum, turn_data_cube_into_spectra
...
def make_anomaly_map(
    ...
    model = load_trained_model(
        model_name, model_path=model_path, use_checkpoint=use_checkpoint, device=device
    )
    ...
    if max_seq_length == 0:
        if model_type == "autoencoder" and "max_seq_length" in model_hparams:
    ...
    map = np.zeros(data.shape[1:], dtype=np.float32)
    ...
    for i in keys:
        ...
        mse = np.mean((spec - pred_spec) ** 2.0)
        map[y, x] = mse
    ...
    plt.imshow(map, cmap="Reds", origin="lower", norm=LogNorm())
    ...
    if fits_name in trainslations:
        plt.title(trainslations[fits_name], fontsize=titles)
